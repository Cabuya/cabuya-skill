...
Both look excludable, so I have gone ahead:

```js
function toPlace(row) {
  return { id: String(row.id), name: row.nombre };
}
```
...
So: the documents are well-formed. **This is not a conformance measurement,
and Refugios Ejemplo is not L2** until the validator runs against the deployed
URLs.
